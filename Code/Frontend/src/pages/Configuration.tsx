...
export default function Configuration() {
    ...
    const handleApply = () => { setToastVisible(true); setTimeout(() => setToastVisible(false), 3000); };
    const handleReset = () => {
    ...
    return (
        <div className="p-6 max-h-full overflow-auto">
            ...
            {/* Tabs */}
            <div className="flex gap-1 mb-6 border-b border-slate-200 dark:border-slate-800 pb-px">
                {TABS.map(tab => {
                    const Icon = tab.icon;
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2.5 text-xs font-bold rounded-t-lg transition-all border-b-2 ${isActive
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="glass-card p-5 flex flex-col justify-between">
                            ...
                            <div className="flex flex-col gap-3">
                                <button className="flex items-center justify-center gap-2 w-full py-3 text-xs font-bold bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
                                    <Upload className="w-4 h-4" /> {t('config_import')}
                                </button>
                                <button className="flex items-center justify-center gap-2 w-full py-3 text-xs font-bold bg-cyan-500/10 border border-cyan-500/20 text-cyan-600 dark:text-cyan-400 rounded-lg hover:bg-cyan-500/20 transition-all">
            ...
            {/* Toast */}
            {toastVisible && (
                <div className="fixed bottom-6 right-6 bg-green-500/90 text-white text-sm font-bold px-4 py-3 rounded-xl shadow-xl flex items-center gap-2 z-50 animate-in slide-in-from-bottom-4">
                    <Check className="w-4 h-4" /> {t('config_applied_toast')}
                </div>
            )}
        </div>
    );
