...
class LiveVisualizer:
    ...
    def __init__(self, simulation):
        ...
        self.fig.text(0.02, 0.5, legend, transform=self.fig.transFigure, fontsize=10, verticalalignment='center')
        
        # View State
        self.is_2d = False
        
        # Enemy and Friend Markers
        self.enemy_scat = self.ax.scatter([], [], [], c='r', marker='x', s=100, label='Enemy')
        self.friend_scat = self.ax.scatter([], [], [], c='g', marker='^', s=100, label='Friend')
        
        # Obstacle Markers (Black Spheres)
        self.obs_scat = self.ax.scatter([], [], [], c='k', marker='o', s=200, label='Obstacle')
    ...
    def on_scroll(self, event):
        ...
        """
        if self.selected_type is None or self.selected_idx == -1: return
        ...
        if self.selected_type == 'enemy':
            if self.selected_idx < len(self.sim.targets):
                self.sim.targets[self.selected_idx][2] = np.clip(self.sim.targets[self.selected_idx][2] + step, 0, BOUNDS[2])
                print(f"Enemy {self.selected_idx} Depth: {self.sim.targets[self.selected_idx][2]:.1f}")
                
        elif self.selected_type == 'friend':
            if self.selected_idx < len(self.sim.friends):
                self.sim.friends[self.selected_idx][2] = np.clip(self.sim.friends[self.selected_idx][2] + step, 0, BOUNDS[2])
                print(f"Friend {self.selected_idx} Depth: {self.sim.friends[self.selected_idx][2]:.1f}")

        elif self.selected_type == 'obstacle':
            if self.selected_idx < len(self.sim.environment.obstacles):
                self.sim.environment.obstacles[self.selected_idx].position[2] = np.clip(self.sim.environment.obstacles[self.selected_idx].position[2] + step, 0, BOUNDS[2])
                print(f"Obstacle {self.selected_idx} Depth: {self.sim.environment.obstacles[self.selected_idx].position[2]:.1f}")

    def get_event_coords(self, event):
        """Helper to get 3D coordinates from event."""
        if event.inaxes != self.ax: return None
    ...
    def on_motion(self, event):
        ...
        if not self.is_dragging or self.selected_idx == -1: return
        ...
        if self.selected_type == 'enemy':
            if self.selected_idx < len(self.sim.targets):
                ...
                self.sim.targets[self.selected_idx] = np.array([coords[0], coords[1], old_z])
                
        elif self.selected_type == 'friend':
            if self.selected_idx < len(self.sim.friends):
                old_z = self.sim.friends[self.selected_idx][2]
                self.sim.friends[self.selected_idx] = np.array([coords[0], coords[1], old_z])
    ...
    def on_key(self, event):
        ...
        """
        if event.key == '1': self.sim.mode = SwarmMode.PATROL
        elif event.key == '2': self.sim.mode = SwarmMode.ATTACK
        elif event.key == '3': self.sim.mode = SwarmMode.DEFEND
        elif event.key == '4': self.sim.mode = SwarmMode.ENCIRCLE
        elif event.key == '6': self.sim.mode = SwarmMode.SHIELD
        elif event.key == '7': self.sim.mode = SwarmMode.SEARCH
        elif event.key == '8': self.sim.mode = SwarmMode.FLASH_EXPANSION
        elif event.key == '9': self.sim.mode = SwarmMode.SCHOOLING
        elif event.key == '0': self.sim.mode = SwarmMode.PREDATOR_PACK
        elif event.key == '-': self.sim.mode = SwarmMode.EXPLORATION
        elif event.key == '5': self.toggle_view()
    ...
    def update(self, frame):
        ...
        # Draw Stems
        for line in self.stem_lines: line.remove()
        self.stem_lines = []
        
        for t in self.sim.targets:
            line, = self.ax.plot([t[0], t[0]], [t[1], t[1]], [0, t[2]], 'r--', alpha=0.5)
            self.stem_lines.append(line)
        for f in self.sim.friends:
            line, = self.ax.plot([f[0], f[0]], [f[1], f[1]], [0, f[2]], 'g--', alpha=0.5)
            self.stem_lines.append(line)
        for o in self.sim.environment.obstacles:
            # Obstacles get a black stem
            line, = self.ax.plot([o.position[0], o.position[0]], [o.position[1], o.position[1]], [0, o.position[2]], 'k--', alpha=0.5)
            self.stem_lines.append(line)
        
        return self.scat, self.enemy_scat, self.friend_scat, self.obs_scat, *self.stem_lines
